...
class AuthenticateViewModel : ObservableObject{
    ...
    func isPassword(userPassword: String) -> Bool{
        let passwordTest = NSPredicate(format: "","^(?=.*\\d)(?=.*[a-z])(?=.*[A-Z])(?!.*\\s).{4,8}$")
        return passwordTest.evaluate(with: userPassword)
    }
    
    func isEmail(userEmail : String) -> Bool{
        let EmailTest = NSPredicate(format: "","^(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\\-+)|([A-Za-z0-9]+\\.+)|([A-Za-z0-9]+\\++))*[A-Za-z0-9]+@((\\w+\\-+)|(\\w+\\.))*\\w{1,63}\\.[a-zA-Z]{2,6}$")
